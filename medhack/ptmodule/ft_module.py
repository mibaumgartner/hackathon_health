...
from medhack.ptmodule.label_smoothing import LabelSmoothing
...
class FineTuneClassificationModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx: Optional[int] = None):
        ...
        self.log("val/acc", acc, prog_bar=False, logger=True, sync_dist=True)
        self.log("val/loss", val_loss, prog_bar=False, logger=True, sync_dist=True)
    ...
    def create_loss(self, loss_name: str) -> torch.nn.Module:
        if loss_name == "CE":
            return LabelSmoothing(smoothing=0.1)
